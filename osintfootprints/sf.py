...
# -------------------------------------------------------------------------------
# Name:         sf
# Purpose:      Main wrapper for calling all OsintFootprints modules
#
...
def main() -> None:
    ...
    sfConfig = {
        ...
        '_genericusers': ",".join(OsintFootprintsHelpers.usernamesFromWordlists(['generic-usernames'])),
        '__database': f"{OsintFootprintsHelpers.dataPath()}/osintfootprints.db",
    ...
    args = None
    p = argparse.ArgumentParser(description=f"OsintFootprints {__version__}: Open Source Intelligence Automation.")
    ...
    p.add_argument("-V", "--version", action='store_true', help="Display the version of OsintFootprints and exit.")
    p.add_argument("-max-threads", type=int, help="Max number of modules to run concurrently.")
    args = p.parse_args()

    if args.version:
        print(f"OsintFootprints {__version__}: Open Source Intelligence Automation.")
        sys.exit(0)
    ...
    log = logging.getLogger(f"osintfootprints.{__name__}")
    ...
    try:
        ...
        sfModules = OsintFootprintsHelpers.loadModulesAsDict(mod_dir, ['sfp_template.py'])
    ...
    try:
        ...
        correlationRulesRaw = OsintFootprintsHelpers.loadCorrelationRulesRaw(correlations_dir, ['template.yaml'])
    except BaseException as e:
    ...
    try:
        dbh = OsintFootprintsDb(sfConfig)
    ...
    if not correlationRulesRaw:
        ...
        try:
            correlator = OsintFootprintsCorrelator(dbh, correlationRulesRaw)
    ...
    if args.correlate:
        ...
        try:
            ...
            corr = OsintFootprintsCorrelator(dbh, correlationRulesRaw, args.correlate)
    ...
    if args.types:
        dbh = OsintFootprintsDb(sfConfig, init=True)
...
def start_scan(sfConfig: dict, sfModules: dict, args, loggingQueue) -> None:
    ...
    Args:
        sfConfig (dict): OsintFootprints config options
        sfModules (dict): modules
        args (argparse.Namespace): command line args
        loggingQueue (Queue): main OsintFootprints logging queue
    """
    log = logging.getLogger(f"osintfootprints.{__name__}")
    ...
    if "." not in target and not target.startswith("+") and '"' not in target:
        target = f"\"{target}\""
    targetType = OsintFootprintsHelpers.targetTypeFromString(target)
    ...
    try:
        p = mp.Process(target=startOsintFootprintsScanner, args=(loggingQueue, scanName, scanId, target, targetType, modlist, cfg))
        p.daemon = True
        p.start()
    except BaseException as e:
...
def start_web_server(sfWebUiConfig: dict, sfConfig: dict, loggingQueue=None) -> None:
    ...
    conf = {
        ...
        '/static': {
            ...
            'tools.staticdir.root': f"{os.path.dirname(os.path.abspath(__file__))}/osintfootprints"
        }
    }

    secrets = dict()
    passwd_file = OsintFootprintsHelpers.dataPath() + '/passwd'
    ...
    if secrets:
        ...
        warn_msg += "Refer to https://www.osintfootprints.net/documentation/#security.\n"
        warn_msg += "********************************************************************\n"
        log.warning(warn_msg)

    using_ssl = False
    key_path = OsintFootprintsHelpers.dataPath() + '/osintfootprints.key'
    crt_path = OsintFootprintsHelpers.dataPath() + '/osintfootprints.crt'
    ...
    print("")
    print("*************************************************************")
    print(" Use OsintFootprints by starting your web browser of choice and ")
    ...
    cherrypy.quickstart(OsintFootprintsWebUi(sfWebUiConfig, sfConfig, loggingQueue), script_name=web_root, config=conf)
...
if __name__ == '__main__':
    if sys.version_info < (3, 7):
        print("OsintFootprints requires Python 3.7 or higher.")
        sys.exit(-1)

    if len(sys.argv) <= 1:
        print("OsintFootprints requires -l <ip>:<port> to start the web server. Try --help for guidance.")
        sys.exit(-1)

    # TODO: remove this after a few releases (added in 3.5 pre-release 2021-09-05)
    from pathlib import Path
    if os.path.exists('osintfootprints.db'):
        print(f"ERROR: osintfootprints.db file exists in {os.path.dirname(__file__)}")
        print("OsintFootprints no longer supports loading the osintfootprints.db database from the application directory.")
        print(f"The database is now loaded from your home directory: {Path.home()}/.osintfootprints/osintfootprints.db")
        print(f"This message will go away once you move or remove osintfootprints.db from {os.path.dirname(__file__)}")
        sys.exit(-1)
    ...
    if os.path.exists('passwd'):
        ...
        print("OsintFootprints no longer supports loading credentials from the application directory.")
        print(f"The passwd file is now loaded from your home directory: {Path.home()}/.osintfootprints/passwd")
