...
class TestOsintFootprintsModuleLoading(unittest.TestCase):
    """
    Test OsintFootprints module loading
    """

    @staticmethod
    def load_modules(sf):
        mod_dir = os.path.dirname(os.path.abspath(__file__)) + '/../../modules/'
        return OsintFootprintsHelpers.loadModulesAsDict(mod_dir, ['sfp_template.py'])
    ...
    def test_module_watched_events_are_valid(self):
        sf = OsintFootprints(self.default_options)
        sf.dbh = OsintFootprintsDb(self.default_options, True)
