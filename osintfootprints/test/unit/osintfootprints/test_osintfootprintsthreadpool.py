...
from osintfootprints import OsintFootprintsThreadPool


@pytest.mark.usefixtures
class TestOsintFootprintsThreadPool(unittest.TestCase):
    """
    ...
    def test_threadPool(self):
        ...
        # Example 1: using map()
        with OsintFootprintsThreadPool(threads) as pool:
            map_results = sorted(
                list(pool.map(
                    callback,
                    iterable,
        ...
        pool = OsintFootprintsThreadPool(threads)
        pool.start()
        for i in iterable2:
