...
OsintFootprints includes various test suites.
...
To run the tests locally, run `./test/run` from the OsintFootprints root directory.
...
Requires OsintFootprints web server to be running on default port (`5001`).
...
To run the tests, start the OsintFootprints web interface on the default port:
