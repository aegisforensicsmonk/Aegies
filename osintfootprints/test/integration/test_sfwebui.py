...
class TestOsintFootprintsWebUiRoutes(helper.CPWebCase):
    @staticmethod
    def setup_server():
        default_config = {
            '_debug': False,  # Debug
            ...
            '__database': f"{OsintFootprintsHelpers.dataPath()}/osintfootprints.test.db",  # note: test database file
        ...
        default_config['__modules__'] = OsintFootprintsHelpers.loadModulesAsDict(mod_dir, ['sfp_template.py'])
        ...
        cherrypy.tree.mount(OsintFootprintsWebUi(default_web_config, default_config), script_name=default_web_config.get('root'), config=conf)
    ...
    def test_static_returns_200(self):
        self.getPage("/static/img/osintfootprints-header.png")
    ...
    def test_optsexport(self):
        ...
        self.assertStatus('200 OK')
        self.assertHeader("Content-Disposition", "attachment; filename=\"OsintFootprints.cfg\"")
    ...
    def test_startscan_unrecognized_scan_target_returns_error(self):
        ...
        self.assertStatus('200 OK')
        self.assertInBody('Invalid target type. Could not recognize it as a target OsintFootprints supports.')

    def test_startscan_invalid_modules_returns_error(self):
        self.getPage("/startscan?scanname=example-scan&scantarget=osintfootprints.net&modulelist=&typelist=&usecase=")
        self.assertStatus('200 OK')
        self.assertInBody('Invalid request: no modules specified for scan.')

    def test_startscan_invalid_typelist_returns_error(self):
        self.getPage("/startscan?scanname=example-scan&scantarget=osintfootprints.net&modulelist=&typelist=doesnotexist&usecase=")
        self.assertStatus('200 OK')
        self.assertInBody('Invalid request: no modules specified for scan.')

    def test_startscan_should_start_a_scan(self):
        self.getPage("/startscan?scanname=osintfootprints.net&scantarget=osintfootprints.net&modulelist=doesnotexist&typelist=doesnotexist&usecase=doesnotexist")
