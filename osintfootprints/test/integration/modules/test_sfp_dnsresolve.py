...
class TestModuleIntegrationDnsResolve(unittest.TestCase):
    ...
    def test_enrichTarget_should_return_OsintFootprintsTarget(self):
        ...
        self.assertIsInstance(result, OsintFootprintsTarget)
    ...
    def test_resolveTargets_should_return_list(self):
        ...
        self.assertIsInstance(resolve_targets, list)
        self.assertIn('osintfootprints.net', resolve_targets)

        target = OsintFootprintsTarget("127.0.0.1", "IP_ADDRESS")
        resolve_targets = module.resolveTargets(target, False)
        self.assertIsInstance(resolve_targets, list)
        self.assertIn('127.0.0.1', resolve_targets)

        target = OsintFootprintsTarget("::1", "IPV6_ADDRESS")
        resolve_targets = module.resolveTargets(target, False)
        self.assertIsInstance(resolve_targets, list)
        self.assertIn('::1', resolve_targets)

        target = OsintFootprintsTarget("127.0.0.1/32", "NETBLOCK_OWNER")
        resolve_targets = module.resolveTargets(target, False)
        self.assertIsInstance(resolve_targets, list)
        self.assertIn('127.0.0.1', resolve_targets)
    ...
    def test_handleEvent_event_data_raw_rir_data_containing_subdomain_should_return_internet_name_event(self):
        ...
        def new_notifyListeners(self, event):
            ...
            expected = "www.osintfootprints.net"
            if str(event.data) != expected:
                raise Exception(f"{event.data} != {expected}")
        ...
        event_data = 'example data www.osintfootprints.net example data'
        event_module = 'example module'
        source_event = evt
        evt = OsintFootprintsEvent(event_type, event_data, event_module, source_event)
