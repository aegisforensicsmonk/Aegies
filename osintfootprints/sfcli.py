...
# -------------------------------------------------------------------------------
# Name:        sfcli
# Purpose:     Command Line Interface for OsintFootprints.
#
...
COPYRIGHT_INFO = "               by Steve Micallef | @osintfootprints\n"
...
class OsintFootprintsCli(cmd.Cmd):
    version = "4.0.0"
    pipecmd = None
    output = None
    modules = []
    ...
    # Make a request to the OsintFootprints server
    def request(self, url, post=None):
        ...
        headers = {
            "User-agent": "OsintFootprints-CLI/" + self.version,
    ...
    def do_ping(self, line):
        """ping
        Ping the OsintFootprints server to ensure it's responding."""
    ...
    # List all OsintFootprints modules.
    ...
    # List all OsintFootprints correlation rules
    ...
    # List all OsintFootprints data element types.
    ...
    def do_load(self, line):
        """load <file>
        Execute OsintFootprints CLI commands found in <file>."""
        pass
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        helpmap = [
            ...
            ["exit", "Exit the OsintFootprints CLI (won't impact running scans)."],
            ["ping", "Test connectivity to the OsintFootprints server."],
            ...
            ["query", "Run SQL against the OsintFootprints SQLite database."],
    ...
    def do_set(self, line):
        """set [opt [= <val>]]
        Set a configuration variable in OsintFootprints."""
        ...
        if not d:
            self.edprint("Unable to obtain OsintFootprints server-side config.")
            return
        ...
        j = json.loads(d)
        if j[0] == "ERROR":
            self.edprint("Error fetching OsintFootprints server-side config.")
            return
        ...
        if val:
            ...
            if not d:
                self.edprint("Unable to set OsintFootprints server-side config.")
                return

            j = json.loads(d)
            if j[0] == "ERROR":
                self.edprint(f"Error setting OsintFootprints server-side config: {j[1]}")
                return
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description='OsintFootprints: Open Source Intelligence Automation.')
    p.add_argument("-d", "--debug", help="Enable debug output.", action='store_true')
    p.add_argument("-s", metavar="URL", type=str, help="Connect to OsintFootprints server on URL. By default, a connection to http://127.0.0.1:5001 will be attempted.")
    p.add_argument("-u", metavar="USER", type=str, help="Username to authenticate to OsintFootprints server.")
    p.add_argument("-p", metavar="PASS", type=str, help="Password to authenticate to OsintFootprints server. Consider using -P PASSFILE instead so that your password isn't visible in your shell history or in process lists!")
    p.add_argument("-P", metavar="PASSFILE", type=str, help="File containing password to authenticate to OsintFootprints server. Ensure permissions on the file are set appropriately!")
    p.add_argument("-e", metavar="FILE", type=str, help="Execute commands from FILE.")
    p.add_argument("-l", metavar="FILE", type=str, help="Log command history to FILE. By default, history is stored to ~/.osintfootprints_history unless disabled with -n.")
    ...
    if args.e:
        ...
        cin = sys.stdin
    s = OsintFootprintsCli(stdin=cin)
    s.identchars += "$"
    ...
    if args.l:
        ...
        try:
            s.ownopts['cli.history_file'] = expanduser("~") + "/.osintfootprints_history"
        except BaseException as e:
            s.dprint(f"Failed to set 'cli.history_file': {e}")
            s.dprint("Using '.osintfootprints_history' in working directory")
            s.ownopts['cli.history_file'] = ".osintfootprints_history"
    if args.o:
