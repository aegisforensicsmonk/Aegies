...
from sfscan import startOsintFootprintsScanner

from osintfootprints import OsintFootprintsDb
from osintfootprints import OsintFootprintsHelpers
from osintfootprints import __version__
from osintfootprints.logger import logListenerSetup, logWorkerSetup

mp.set_start_method("spawn", force=True)


class OsintFootprintsWebUi:
    """OsintFootprints web interface."""
    ...
    def __init__(self: 'OsintFootprintsWebUi', web_config: dict, config: dict, loggingQueue: 'logging.handlers.QueueListener' = None) -> None:
        """Initialize web server.

        Args:
            web_config (dict): config settings for web interface (interface, port, root path)
            config (dict): OsintFootprints config
            loggingQueue: TBD
        ...
        dbh = OsintFootprintsDb(self.defaultConfig, init=True)
        sf = OsintFootprints(self.defaultConfig)
        ...
        self.log = logging.getLogger(f"osintfootprints.{__name__}")
        ...
        headers = secure_headers.framework.cherrypy()
        # Allow embedding in our frontend iframe
        headers = [h for h in headers if h[0] != 'X-Frame-Options']
    ...
    def error_page(self: 'OsintFootprintsWebUi') -> None:
        """Error page."""
    ...
    def error_page_401(self: 'OsintFootprintsWebUi', status: str, message: str, traceback: str, version: str) -> str:
    ...
    def error_page_404(self: 'OsintFootprintsWebUi', status: str, message: str, traceback: str, version: str) -> str:
        ...
        """
        templ = Template(filename='osintfootprints/templates/error.tmpl', lookup=self.lookup)
        return templ.render(message='Not Found', docroot=self.docroot, status=status, version=__version__)

    def jsonify_error(self: 'OsintFootprintsWebUi', status: str, message: str) -> dict:
    ...
    def error(self: 'OsintFootprintsWebUi', message: str) -> None:
    ...
    def cleanUserInput(self: 'OsintFootprintsWebUi', inputList: list) -> list:
    ...
    def searchBase(self: 'OsintFootprintsWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search.
        ...
        dbh = OsintFootprintsDb(self.config)
        criteria = {
            'scan_id': id or '',
            'type': eventType or '',
            'value': value or '',
    ...
    def buildExcel(self: 'OsintFootprintsWebUi', data: list, columnNames: list, sheetNameIndex: int = 0) -> str:
    ...
    @cherrypy.expose
    def scanexportlogs(self: 'OsintFootprintsWebUi', id: str, dialect: str = "excel") -> bytes:
        """Get scan log
        ...
        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename=OsintFootprints-{id}.log.csv"
        cherrypy.response.headers['Content-Type'] = "application/csv"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return fileobj.getvalue().encode('utf-8')

    @cherrypy.expose
    def scancorrelationsexport(self: 'OsintFootprintsWebUi', id: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if scan_name:
                fname = f"{scan_name}-OsintFootprints-correlations.xlxs"
            else:
                fname = "OsintFootprints-correlations.xlxs"
        ...
        if filetype.lower() == 'csv':
            ...
            if scan_name:
                fname = f"{scan_name}-OsintFootprints-correlations.csv"
            else:
                fname = "OsintFootprints-correlations.csv"
    ...
    @cherrypy.expose
    def scaneventresultexport(self: 'OsintFootprintsWebUi', id: str, type: str, filetype: str = "csv", dialect: str = "excel") -> str:
    ...
    @cherrypy.expose
    def scaneventresultexportmulti(self: 'OsintFootprintsWebUi', ids: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-OsintFootprints.xlsx"
        ...
        if filetype.lower() == 'csv':
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-OsintFootprints.csv"
    ...
    @cherrypy.expose
    def scansearchresultexport(self: 'OsintFootprintsWebUi', id: str, eventType: str = None, value: str = None, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=OsintFootprints.xlsx"
            cherrypy.response.headers['Content-Type'] = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return self.buildExcel(rows, ["Updated", "Type", "Module", "Source",
                                   "F/P", "Data"], sheetNameIndex=1)
        ...
        if filetype.lower() == 'csv':
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=OsintFootprints.csv"
            cherrypy.response.headers['Content-Type'] = "application/csv"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return fileobj.getvalue().encode('utf-8')
    ...
    @cherrypy.expose
    def scanexportjsonmulti(self: 'OsintFootprintsWebUi', ids: str) -> str:
        ...
        if len(ids.split(',')) > 1 or scan_name == "":
            fname = "OsintFootprints.json"
        else:
            fname = scan_name + "-OsintFootprints.json"
    ...
    @cherrypy.expose
    def scanviz(self: 'OsintFootprintsWebUi', id: str, gexf: str = "0") -> str:
        ...
        if gexf == "0":
            return OsintFootprintsHelpers.buildGraphJson([root], data)

        if not scan_name:
            fname = "OsintFootprints.gexf"
        else:
            fname = scan_name + "OsintFootprints.gexf"

        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename={fname}"
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return OsintFootprintsHelpers.buildGraphGexf([root], "OsintFootprints Export", data)

    @cherrypy.expose
    def scanvizmulti(self: 'OsintFootprintsWebUi', ids: str, gexf: str = "1") -> str:
        ...
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return OsintFootprintsHelpers.buildGraphGexf(roots, "OsintFootprints Export", data)

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanopts(self: 'OsintFootprintsWebUi', id: str) -> dict:
    ...
    @cherrypy.expose
    def rerunscan(self: 'OsintFootprintsWebUi', id: str) -> None:
        """Rerun a scan.
        ...
        targetType = OsintFootprintsHelpers.targetTypeFromString(scantarget)
        if not targetType:
            ...
            # target type again.
            targetType = OsintFootprintsHelpers.targetTypeFromString(f'"{scantarget}"')

        if targetType not in ["HUMAN_NAME", "BITCOIN_ADDRESS"]:
            scantarget = scantarget.lower()

        # Start running a new scan
        scanId = OsintFootprintsHelpers.genScanInstanceId()
        try:
            p = mp.Process(target=startOsintFootprintsScanner, args=(self.loggingQueue, scanname, scanId, scantarget, targetType, modlist, cfg))
            p.daemon = True
            p.start()
    ...
    @cherrypy.expose
    def rerunscanmulti(self: 'OsintFootprintsWebUi', ids: str) -> str:
        """Rerun scans.
        ...
        for id in ids.split(","):
            ...
            if targetType is None:
                ...
                return self.error("Invalid target type. Could not recognize it as a target OsintFootprints supports.")
        ...
        templ = Template(filename='osintfootprints/templates/scanlist.tmpl', lookup=self.lookup)
        return templ.render(rerunscans=True, docroot=self.docroot, pageid="SCANLIST", version=__version__)

    @cherrypy.expose
    def newscan(self: 'OsintFootprintsWebUi') -> str:
        ...
        templ = Template(filename='osintfootprints/templates/newscan.tmpl', lookup=self.lookup)
        return templ.render(pageid='NEWSCAN', types=types, docroot=self.docroot,
                            modules=self.config['__modules__'], scanname="",
                            selectedmods="", scantarget="", version=__version__)

    @cherrypy.expose
    def clonescan(self: 'OsintFootprintsWebUi', id: str) -> str:
    ...
    @cherrypy.expose
    def index(self: 'OsintFootprintsWebUi') -> str:
        """Show scan list page.
    ...
    @cherrypy.expose
    def scaninfo(self: 'OsintFootprintsWebUi', id: str) -> str:
        ...
        templ = Template(filename='osintfootprints/templates/scaninfo.tmpl', lookup=self.lookup, input_encoding='utf-8')
        return templ.render(id=id, name=html.escape(res[0]), status=res[5], docroot=self.docroot, version=__version__,
                            pageid="SCANLIST")

    @cherrypy.expose
    def opts(self: 'OsintFootprintsWebUi', updated: str = None) -> str:
        ...
        """
        templ = Template(filename='osintfootprints/templates/opts.tmpl', lookup=self.lookup)
    ...
    @cherrypy.expose
    def optsexport(self: 'OsintFootprintsWebUi', pattern: str = None) -> str:
        ...
        """
        sf = OsintFootprints(self.config)
        ...
        cherrypy.response.headers['Content-Disposition'] = 'attachment; filename="OsintFootprints.cfg"'
        cherrypy.response.headers['Content-Type'] = "text/plain"
        return content

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def optsraw(self: 'OsintFootprintsWebUi') -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scandelete(self: 'OsintFootprintsWebUi', id: str) -> str:
        """Delete scan(s).
    ...
    @cherrypy.expose
    def savesettings(self: 'OsintFootprintsWebUi', allopts: str, token: str, configFile: 'cherrypy._cpreqbody.Part' = None) -> None:
        ...
        if configFile and configFile.file:
            try:
                ...
                return self.error(f"Failed to parse input file. Was it generated from OsintFootprints? ({e})")
    ...
    @cherrypy.expose
    def savesettingsraw(self: 'OsintFootprintsWebUi', allopts: str, token: str) -> str:
        """Save settings, also used to completely reset them to default.
    ...
    def reset_settings(self: 'OsintFootprintsWebUi') -> bool:
    ...
    @cherrypy.expose
    def resultsetfp(self: 'OsintFootprintsWebUi', id: str, resultids: str, fp: str) -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def eventtypes(self: 'OsintFootprintsWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def modules(self: 'OsintFootprintsWebUi') -> list:
        """List all modules.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def correlationrules(self: 'OsintFootprintsWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def ping(self: 'OsintFootprintsWebUi') -> list:
        """For the CLI to test connectivity to this server.

        Returns:
            list: OsintFootprints version as JSON
        """
        return ["SUCCESS", __version__]

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def query(self: 'OsintFootprintsWebUi', query: str) -> str:
    ...
    @cherrypy.expose
    def startscan(self: 'OsintFootprintsWebUi', scanname: str, scantarget: str, modulelist: str, typelist: str, usecase: str) -> str:
        """Initiate a scan.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def stopscan(self: 'OsintFootprintsWebUi', id: str) -> str:
        """Stop a scan.
        ...
        for scan_id in ids:
            ...
            if scan_status != "RUNNING" and scan_status != "STARTING":
                return self.jsonify_error('400', f"The running scan is currently in the state '{scan_status}', please try again later or restart OsintFootprints.")
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlog(self: 'OsintFootprintsWebUi', id: str, limit: str = None, rowId: str = None, reverse: str = None) -> list:
        """Scan log data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanerrors(self: 'OsintFootprintsWebUi', id: str, limit: str = None) -> list:
        """Scan error data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlist(self: 'OsintFootprintsWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanstatus(self: 'OsintFootprintsWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scansummary(self: 'OsintFootprintsWebUi', id: str, by: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scancorrelations(self: 'OsintFootprintsWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresults(self: 'OsintFootprintsWebUi', id: str, eventType: str = None, filterfp: bool = False, correlationId: str = None) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresultsunique(self: 'OsintFootprintsWebUi', id: str, eventType: str, filterfp: bool = False) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def search(self: 'OsintFootprintsWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search scans.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanhistory(self: 'OsintFootprintsWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanelementtypediscovery(self: 'OsintFootprintsWebUi', id: str, eventType: str) -> dict:
        ...
        del pc['ROOT']
        retdata['tree'] = OsintFootprintsHelpers.dataParentChildToTree(pc)
