...
class OsintFootprintsDb:
    """OsintFootprints database
    ...
    # Queries for creating the OsintFootprints database
    createSchemaQueries = [
    ...
    eventDetails = [
        ['ROOT', 'Internal OsintFootprints Root event', 1, 'INTERNAL'],
        ...
        with self.dbhLock:
            try:
                ...
                try:
                    ...
                    raise IOError("Tried to set up the OsintFootprints database schema, but failed") from e
            ...
            try:
                ...
                try:
                    ...
                    raise IOError("Looks like you are running a pre-4.0 database. Unfortunately "
                                  "OsintFootprints wasn't able to migrate you, so you'll need to delete "
                                  "your OsintFootprints database in order to proceed.") from None
    ...
    def scanEventStore(self, instanceId: str, sfEvent, truncateSize: int = 0) -> None:
        ...
        Args:
            instanceId (str): scan instance ID
            sfEvent (OsintFootprintsEvent): event to be stored in the database
        ...
        if not isinstance(sfEvent, OsintFootprintsEvent):
            raise TypeError(f"sfEvent is {type(sfEvent)}; expected OsintFootprintsEvent()") from None
        ...
        if not isinstance(sfEvent.sourceEvent, OsintFootprintsEvent) and sfEvent.eventType != "ROOT":
