...
from osintfootprints import OsintFootprintsDb, OsintFootprintsHelpers


class OsintFootprintsSqliteLogHandler(logging.Handler):
    ...
    def makeDbh(self) -> None:
        """TBD."""
        self.dbh = OsintFootprintsDb(self.opts)


def logListenerSetup(loggingQueue, opts: dict = None) -> 'logging.handlers.QueueListener':
    """Create and start a OsintFootprints log listener in its own thread.
    ...
    Args:
        ...
        opts (dict): OsintFootprints config

    Returns:
        osintfootprintsLogListener (logging.handlers.QueueListener): Log listener
    """
    if opts is None:
        opts = dict()
    ...
    log_dir = OsintFootprintsHelpers.logPath()
    debug_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/osintfootprints.debug.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    error_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/osintfootprints.error.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    if doLogging and opts is not None:
        sqlite_handler = OsintFootprintsSqliteLogHandler(opts)
        sqlite_handler.setLevel(logLevel)
        sqlite_handler.setFormatter(log_format)
        handlers.append(sqlite_handler)
    osintfootprintsLogListener = QueueListener(loggingQueue, *handlers)
    osintfootprintsLogListener.start()
    atexit.register(stop_listener, osintfootprintsLogListener)
    return osintfootprintsLogListener


def logWorkerSetup(loggingQueue) -> 'logging.Logger':
    """Root OsintFootprints logger.
    ...
    """
    log = logging.getLogger("osintfootprints")
