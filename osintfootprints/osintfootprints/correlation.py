...
class OsintFootprintsCorrelator:
    """OsintFootprints correlation capabilities.

    Todo:
        Make the rule checking per analysis method
    """

    log = logging.getLogger("osintfootprints.correlator")
    dbh = None
    scanId = None
    types = None
    rules = list()
    ...
    def __init__(self, dbh: OsintFootprintsDb, ruleset: dict, scanId: str = None) -> None:
        """Initialize OsintFootprints correlator engine with scan ID and ruleset.
        ...
        if not isinstance(dbh, OsintFootprintsDb):
            raise TypeError(f"dbh is {type(dbh)}; expected OsintFootprintsDb()")
    ...
    def build_db_criteria(self, matchrule: dict) -> dict:
        ...
        Returns:
            dict: criteria to be used with OsintFootprintsDb.scanResultEvent()
