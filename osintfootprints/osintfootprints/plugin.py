...
from .threadpool import OsintFootprintsThreadPool
...
class OsintFootprintsPluginLogger(logging.Logger):
    """Used only in OsintFootprintsPlugin to prevent modules
...
class OsintFootprintsPlugin():
    """OsintFootprintsPlugin module object
    ...
    Attributes:
        ...
        _currentEvent (OsintFootprintsEvent): Current event being processed
    ...
    __scanId__ = None
    # (only used in OsintFootprints HX) tracking of data sources
    __dataSource__ = None
    ...
    # OsintFootprints object, set in each module's setup() function
    sf = None
    ...
    def log(self):
        if self._log is None:
            logging.setLoggerClass(OsintFootprintsPluginLogger)  # temporarily set logger class
            self._log = logging.getLogger(f"osintfootprints.{self.__name__}")  # init OsintFootprintsPluginLogger
    ...
    def setup(self, sf, userOpts: dict = {}) -> None:
        ...
        Args:
            sf (OsintFootprints): OsintFootprints object
            userOpts (dict): TBD
        """
        pass
    ...
    def setTarget(self, target) -> None:
        ...
        Args:
            target (OsintFootprintsTarget): target

        Raises:
            TypeError: target argument was invalid type
        """
        from osintfootprints import OsintFootprintsTarget

        if not isinstance(target, OsintFootprintsTarget):
            raise TypeError(f"target is {type(target)}; expected OsintFootprintsTarget")
    ...
    def setDbh(self, dbh) -> None:
        ...
        Args:
            dbh (OsintFootprintsDb): database handle
        """
        self.__sfdb__ = dbh
    ...
    def tempStorage(self) -> dict:
        ...
        Note:
            Required for OsintFootprints HX compatibility of modules.
    ...
    def notifyListeners(self, sfEvent) -> None:
        ...
        Args:
            sfEvent (OsintFootprintsEvent): event

        Raises:
            TypeError: sfEvent argument was invalid type
        """

        from osintfootprints import OsintFootprintsEvent

        if not isinstance(sfEvent, OsintFootprintsEvent):
            raise TypeError(f"sfEvent is {type(sfEvent)}; expected OsintFootprintsEvent")
    ...
    def threadWorker(self) -> None:
        try:
            ...
            self.setDbh(OsintFootprintsDb(self.opts))
