...
# -------------------------------------------------------------------------------
# Name:         sfp_seon
# Purpose:      Osintfootprints plugin to query seon.io to gather intelligence about
...
from osintfootprints import OsintFootprintsEvent, OsintFootprintsPlugin


class sfp_seon(OsintFootprintsPlugin):
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            if resultSet:
                if resultSet.get('score', 0) >= self.opts['fraud_threshold']:
                    ...
                    evt = OsintFootprintsEvent("MALICIOUS_IPADDR", maliciousDesc, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                    if resultSet.get('tor'):
                        evt = OsintFootprintsEvent("WEBSERVER_TECHNOLOGY", f"Server is TOR node: {resultSet.get('tor')}", self.__name__, event)
                        self.notifyListeners(evt)

                        evt = OsintFootprintsEvent("TOR_EXIT_NODE", eventData, self.__name__, event)
                        self.notifyListeners(evt)

                    if resultSet.get('vpn'):
                        evt = OsintFootprintsEvent("WEBSERVER_TECHNOLOGY", f"Server is VPN: {resultSet.get('vpn')}", self.__name__, event)
                        self.notifyListeners(evt)

                        evt = OsintFootprintsEvent("VPN_HOST", eventData, self.__name__, event)
                        self.notifyListeners(evt)

                    if resultSet.get('web_proxy'):
                        evt = OsintFootprintsEvent("WEBSERVER_TECHNOLOGY", f"Server is Web Proxy: {resultSet.get('web_proxy')}", self.__name__, event)
                        self.notifyListeners(evt)

                        evt = OsintFootprintsEvent("PROXY_HOST", eventData, self.__name__, event)
                        self.notifyListeners(evt)

                    if resultSet.get('public_proxy'):
                        evt = OsintFootprintsEvent("WEBSERVER_TECHNOLOGY", f"Server is Public Proxy: {resultSet.get('public_proxy')}", self.__name__, event)
                        self.notifyListeners(evt)
                ...
                if resultSet.get('country'):
                    ...
                    evt = OsintFootprintsEvent('GEOINFO', location, self.__name__, event)
                    self.notifyListeners(evt)

                    evt = OsintFootprintsEvent('PHYSICAL_COORDINATES', f"{resultSet.get('latitude')}, {resultSet.get('longitude')}", self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                if resultSet.get('open_ports'):
                    for port in resultSet.get('open_ports'):
                        evt = OsintFootprintsEvent('TCP_PORT_OPEN', f"{eventData}:{port}", self.__name__, event)
                        self.notifyListeners(evt)
                        dataFound = True

                if dataFound:
                    evt = OsintFootprintsEvent('RAW_RIR_DATA', str(resultSet), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if resultSet:
                if resultSet.get('score') >= self.opts['fraud_threshold']:
                    maliciousDesc = f"SEON [{eventData}]\n - FRAUD SCORE: {resultSet.get('score')}"
                    evt = OsintFootprintsEvent("MALICIOUS_EMAILADDR", maliciousDesc, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                if resultSet.get('deliverable'):
                    evt = OsintFootprintsEvent("EMAILADDR_DELIVERABLE", eventData, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True
                else:
                    evt = OsintFootprintsEvent("EMAILADDR_UNDELIVERABLE", eventData, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                if resultSet.get('domain_details'):
                    if resultSet.get('domain_details').get('disposable'):
                        evt = OsintFootprintsEvent("EMAILADDR_DISPOSABLE", eventData, self.__name__, event)
                        self.notifyListeners(evt)
                        dataFound = True
                    ...
                    for site in socialMediaList:
                        if resultSet.get('account_details').get(site):
                            if resultSet.get('account_details').get(site).get('url'):
                                evt = OsintFootprintsEvent("SOCIAL_MEDIA", f"{site}: <SFURL>{resultSet.get('account_details').get(site).get('url')}</SFURL>", self.__name__, event)
                                self.notifyListeners(evt)
                            elif resultSet.get('account_details').get(site).get('registered'):
                                evt = OsintFootprintsEvent("SOCIAL_MEDIA", f"Registered on {site}", self.__name__, event)
                                self.notifyListeners(evt)
                            dataFound = True

                            if site == 'linkedin':
                                if resultSet.get('account_details').get(site).get('company'):
                                    evt = OsintFootprintsEvent("COMPANY_NAME", resultSet.get('account_details').get(site).get('company'), self.__name__, event)
                                    self.notifyListeners(evt)
                                    dataFound = True

                                if resultSet.get('account_details').get(site).get('name'):
                                    evt = OsintFootprintsEvent("HUMAN_NAME", resultSet.get('account_details').get(site).get('name'), self.__name__, event)
                                    self.notifyListeners(evt)
                                    dataFound = True
                ...
                if resultSet.get('breach_details').get('breaches'):
                    ...
                    for breachSet in breachList:
                        evt = OsintFootprintsEvent("EMAILADDR_COMPROMISED", f"{eventData} [{breachSet.get('name', 'Unknown')}]", self.__name__, event)
                        self.notifyListeners(evt)
                        dataFound = True
            ...
            if resultSet:
                if resultSet.get('score') >= self.opts['fraud_threshold']:
                    maliciousDesc = f"SEON [{eventData}]\n - FRAUD SCORE: {resultSet.get('score')}"
                    evt = OsintFootprintsEvent("MALICIOUS_PHONE_NUMBER", maliciousDesc, self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True
                ...
                if resultSet.get('type'):
                    evt = OsintFootprintsEvent("PHONE_NUMBER_TYPE", resultSet.get('type'), self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True

                if resultSet.get('carrier'):
                    evt = OsintFootprintsEvent("PROVIDER_TELCO", resultSet.get('carrier'), self.__name__, event)
                    self.notifyListeners(evt)
                    dataFound = True
