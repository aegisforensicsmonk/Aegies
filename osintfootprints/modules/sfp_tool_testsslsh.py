...
# Purpose:     OsintFootprints plug-in for using the testssl.sh tool.
...
from osintfootprints import OsintFootprintsPlugin, OsintFootprintsEvent, OsintFootprintsHelpers


class sfp_tool_testsslsh(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if not OsintFootprintsHelpers.sanitiseInput(eventData, extra=['/']):
        ...
        for target in targets:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                if generate:
                    pevent = OsintFootprintsEvent("IP_ADDRESS", target, self.__name__, event)
            ...
            for result in result_json:
                ...
                if 'cve' in result:
                    ...
                    evt = OsintFootprintsEvent("VULNERABILITY_GENERAL", f"{result['id']} ({result['finding']})", self.__name__, pevent)
                    self.notifyListeners(evt)
