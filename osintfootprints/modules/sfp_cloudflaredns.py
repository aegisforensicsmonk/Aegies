...
class sfp_cloudflaredns(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if '0.0.0.0' not in malware:
            ...
            evt = OsintFootprintsEvent(e, f"CloudFlare - Family [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)
            return

        # Host is blocked only by malware filters
        self.debug(f"{eventData} blocked by CloudFlare Malware DNS")
        evt = OsintFootprintsEvent(e, f"CloudFlare - Malware [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)
