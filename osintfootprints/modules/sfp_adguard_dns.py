...
# Purpose:     OsintFootprints plug-in for looking up whether hosts are blocked by
#              AdGuard DNS servers.
#
...
class sfp_adguard_dns(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if '94.140.14.35' in family:
            ...
            evt = OsintFootprintsEvent(blacklist_type, f"AdGuard - Family Filter [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)

        if '94.140.14.35' in default:
            self.debug(f"{eventData} blocked by AdGuard Default DNS")
            evt = OsintFootprintsEvent(blacklist_type, f"AdGuard - Default Filter [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)
