...
class sfp_emailrep(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if credentials_leaked:
            evt = OsintFootprintsEvent('EMAILADDR_COMPROMISED', eventData + " [Unknown]", self.__name__, event)
            self.notifyListeners(evt)

        malicious_activity = details.get('malicious_activity')
        if malicious_activity:
            evt = OsintFootprintsEvent('MALICIOUS_EMAILADDR', 'EmailRep [' + eventData + ']', self.__name__, event)
            self.notifyListeners(evt)
