...
class sfp_intelx(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        for info in data:
            for rec in info.get("records", dict()):
                ...
                e = OsintFootprintsEvent(evt, val, self.__name__, event)
        ...
        for info in data:
            for rec in info.get("selectors", dict()):
                ...
                if evt == "INTERNET_NAME" and self.sf.isDomain(val, self.opts['_internettlds']):
                    e = OsintFootprintsEvent("DOMAIN_NAME", val, self.__name__, event)
                    self.notifyListeners(e)
