...
# -------------------------------------------------------------------------------
# Name:         sfp_tool_nmap
# Purpose:      OsintFootprints plug-in for using nmap to perform OS fingerprinting.
#
...
class sfp_tool_nmap(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "IP_ADDRESS":
            try:
                ...
                if opsys:
                    evt = OsintFootprintsEvent("OPERATING_SYSTEM", opsys, self.__name__, event)
            ...
            try:
                ...
                for line in content.split('\n'):
                    ...
                    if opsys and currentIp:
                        ipevent = OsintFootprintsEvent("IP_ADDRESS", currentIp, self.__name__, event)
                        self.notifyListeners(ipevent)

                        evt = OsintFootprintsEvent("OPERATING_SYSTEM", opsys, self.__name__, ipevent)
                        self.notifyListeners(evt)
                        currentIp = None
            except Exception as e:
