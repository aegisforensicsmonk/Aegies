...
class sfp_wikileaks(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        links = dict()
        p = OsintFootprintsHelpers.extractLinksFromHtml(wlurl, res['content'], "wikileaks.org")
        if p:
            links.update(p)

        p = OsintFootprintsHelpers.extractLinksFromHtml(wlurl, res['content'], "cryptome.org")
        if p:
            links.update(p)
            ...
            for link in links:
                ...
                if link.count('/') >= 4:
                    if not link.endswith(".js") and not link.endswith(".css"):
                        evt = OsintFootprintsEvent("LEAKSITE_URL", link, self.__name__, event)
