...
class sfp_ripe(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "BGP_AS_OWNER":
            ...
            for netblock in netblocks:
                ...
                if ":" in netblock:
                    evt = OsintFootprintsEvent("NETBLOCKV6_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = OsintFootprintsEvent("NETBLOCK_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)

            evt = OsintFootprintsEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if eventName.startswith("NETBLOCK"):
            ...
            if eventName in ["NETBLOCK_OWNER", "NETBLOCKV6_OWNER"] and self.ownsAs(asn):
                asevt = OsintFootprintsEvent("BGP_AS_OWNER", asn, self.__name__, event)
                self.notifyListeners(asevt)
                evt = OsintFootprintsEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
                self.notifyListeners(evt)
            else:
                asevt = OsintFootprintsEvent("BGP_AS_MEMBER", asn, self.__name__, event)
                self.notifyListeners(asevt)
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            if ":" in prefix:
                evt = OsintFootprintsEvent("NETBLOCKV6_" + relationship, prefix, self.__name__, event)
            else:
                evt = OsintFootprintsEvent("NETBLOCK_" + relationship, prefix, self.__name__, event)
            self.notifyListeners(evt)
