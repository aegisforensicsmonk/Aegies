...
class sfp_haveibeenpwned(OsintFootprintsPlugin):
    ...
    def query(self, qry):
        ...
        while retry < 2:
            ...
            res = self.sf.fetchUrl(url, timeout=self.opts['_fetchtimeout'],
                                   useragent="OsintFootprints", headers=hdrs)
    ...
    def handleEvent(self, event):
        ...
        if data is not None:
            for n in data:
                ...
                if eventName == 'EMAILADDR':
                    e = OsintFootprintsEvent("EMAILADDR_COMPROMISED", eventData + " [" + site + "]",
                                        self.__name__, event)
                else:
                    e = OsintFootprintsEvent("PHONE_NUMBER_COMPROMISED", eventData + " [" + site + "]",
                                        self.__name__, event)
        ...
        for link in links:
            try:
                ...
                evt1 = OsintFootprintsEvent("LEAKSITE_URL", link, self.__name__, event)
                self.notifyListeners(evt1)

                evt2 = OsintFootprintsEvent("LEAKSITE_CONTENT", res['content'], self.__name__, evt1)
