...
class sfp_urlscan(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        evt = OsintFootprintsEvent('RAW_RIR_DATA', str(results), self.__name__, event)
        ...
        for domain in set(domains):
            if self.opts['verify'] and not self.sf.resolveHost(domain) and not self.sf.resolveHost6(domain):
                evt = OsintFootprintsEvent('INTERNET_NAME_UNRESOLVED', domain, self.__name__, event)
                self.notifyListeners(evt)
            else:
                evt = OsintFootprintsEvent('INTERNET_NAME', domain, self.__name__, event)
                self.notifyListeners(evt)
        ...
        for asn in set(asns):
            evt = OsintFootprintsEvent('BGP_AS_MEMBER', asn, self.__name__, event)
            self.notifyListeners(evt)

        for server in set(servers):
            evt = OsintFootprintsEvent('WEBSERVER_BANNER', server, self.__name__, event)
            self.notifyListeners(evt)
