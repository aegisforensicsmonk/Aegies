...
class sfp_yandexdns(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        for result in res:
            ...
            evt = OsintFootprintsEvent(blacklist_type, f"{self.checks[k]} [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)

            if k == '213.180.193.250':
                evt = OsintFootprintsEvent(malicious_type, f"{self.checks[k]} [{eventData}]", self.__name__, event)
                self.notifyListeners(evt)
