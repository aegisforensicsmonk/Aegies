...
# -------------------------------------------------------------------------------
# Name:         sfp_pgp
# Purpose:      OsintFootprints plug-in for looking up e-mail addresses in PGP
...
class sfp_pgp(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName in ["DOMAIN_NAME", "INTERNET_NAME"]:
            ...
            emails = OsintFootprintsHelpers.extractEmailsFromText(res['content'])
        ...
        if eventName == "EMAILADDR" and self.opts['retrieve_keys']:
            ...
            keys = OsintFootprintsHelpers.extractPgpKeysFromText(res['content'])
            self.info(f"Found {len(keys)} public PGP keys")

            for key in keys:
                self.debug(f"Found public key: {key}")
                evt = OsintFootprintsEvent("PGP_KEY", key, self.__name__, event)
                self.notifyListeners(evt)
