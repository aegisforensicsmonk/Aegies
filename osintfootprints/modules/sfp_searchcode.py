...
class sfp_searchcode(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        while page < max_pages:
            ...
            emails = OsintFootprintsHelpers.extractEmailsFromText(str(results))
            for email in emails:
                ...
                evt = OsintFootprintsEvent(evt_type, email, self.__name__, event)
            ...
            for result in results:
                ...
                if lines:
                    for line in lines:
                        links.update(OsintFootprintsHelpers.extractUrlsFromText(lines[line]))
            ...
            for link in links:
                ...
                evt = OsintFootprintsEvent('LINKED_URL_INTERNAL', link, self.__name__, event)
                self.notifyListeners(evt)
                ...
                if self.opts['dns_resolve'] and not self.sf.resolveHost(host) and not self.sf.resolveHost6(host):
                    ...
                    evt = OsintFootprintsEvent("INTERNET_NAME_UNRESOLVED", host, self.__name__, event)
                    self.notifyListeners(evt)
                else:
            ...
            for result in results:
                ...
                evt = OsintFootprintsEvent('PUBLIC_CODE_REPO', repo_data, self.__name__, event)
                self.notifyListeners(evt)

                evt = OsintFootprintsEvent('RAW_RIR_DATA', json.dumps(result), self.__name__, event)
                self.notifyListeners(evt)
