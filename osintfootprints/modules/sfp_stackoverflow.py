...
class sfp_stackoverflow(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        for item in items:
            ...
            emails = OsintFootprintsHelpers.extractEmailsFromText(text)
            if emails:
                for email in emails:
        ...
        for email in set(allEmails):
            ...
            if self.getTarget().matches(email):
                e = OsintFootprintsEvent('EMAILADDR', email, self.__name__, event)
            else:
                e = OsintFootprintsEvent('AFFILIATE_EMAILADDR', email, self.__name__, event)
            self.notifyListeners(e)

        for username in set(allUsernames):
            if " " in username:
                e = OsintFootprintsEvent('RAW_RIR_DATA', 'Possible full name: ' + username, self.__name__, event)
            else:
                e = OsintFootprintsEvent('USERNAME', username, self.__name__, event)
            self.notifyListeners(e)

        for ip in set(allIP4s):
            ip = str(ip)
            e = OsintFootprintsEvent('AFFILIATE_IP_ADDRESS', ip, self.__name__, event)
            self.notifyListeners(e)

        for ip in set(allIP6s):
            ip = str(ip)
            e = OsintFootprintsEvent('AFFILIATE_IPV6_ADDRESS', ip, self.__name__, event)
            self.notifyListeners(e)
