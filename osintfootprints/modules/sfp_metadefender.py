...
class sfp_metadefender(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'IP_ADDRESS':
            ...
            for m in sources:
                ...
                evt = OsintFootprintsEvent('MALICIOUS_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = OsintFootprintsEvent('BLACKLISTED_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName == 'INTERNET_NAME':
            ...
            for m in sources:
                ...
                if m['assessment'] == "trustworthy":
                    continue
                provider = m.get('provider')
                evt = OsintFootprintsEvent('MALICIOUS_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = OsintFootprintsEvent('BLACKLISTED_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
