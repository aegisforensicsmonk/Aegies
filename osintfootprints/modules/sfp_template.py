...
class sfp_template(OsintFootprintsPlugin):
    ...
    meta = {
        ...
        'summary': "This is an example module to help developers create their own OsintFootprints modules.",
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        # instead since on OsintFootprints HX, different mechanisms are used to persist
        ...
        # utilised in OsintFootprints HX and not the open source version.
    ...
    # For a list of all events, check osintfootprints/db.py.
    ...
    def query(self, qry):
        ...
        # from global config), and the user agent is OsintFootprints so that the
        ...
        # - fatal(message) if it's a horrible thing and should kill OsintFootprints completely
    ...
    def handleEvent(self, event):
        # The three most used fields in OsintFootprintsEvent are:
        ...
        for addr in qrylist:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                # to process and is a fundamental part of the OsintFootprints architecture.
                ...
                # event of this event. This enables OsintFootprints to link events so users
                # can see what events generated other events, seeing a full chain of
                # discovery from their target to the data returned here.
                pevent = OsintFootprintsEvent("IP_ADDRESS", addr, self.__name__, event)
                ...
                pevent = OsintFootprintsEvent("AFFILIATE_IPADDR", addr, self.__name__, event)
                self.notifyListeners(pevent)
            else:
            ...
            evt = OsintFootprintsEvent("RAW_RIR_DATA", str(rec), self.__name__, pevent)
            ...
            os = rec.get('os')
            if os:
                evt = OsintFootprintsEvent("OPERATING_SYSTEM", f"{os} ({addr})", self.__name__, pevent)
                self.notifyListeners(evt)
