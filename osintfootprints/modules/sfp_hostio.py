...
class sfp_hostio(OsintFootprintsPlugin):
        ...
        if ipinfo and isinstance(ipinfo, dict):
            for address, ip_data in data["ipinfo"].items():
                ...
                if not self.sf.validIP(address):
                    continue
                evt = OsintFootprintsEvent("IP_ADDRESS", address, self.__name__, event)
                self.notifyListeners(evt)
                found = True

                loc = ip_data.get("loc")
                if loc and isinstance(loc, str):
                    loc_evt = OsintFootprintsEvent(
                ...
                if geo_info:
                    geo_info_evt = OsintFootprintsEvent(
