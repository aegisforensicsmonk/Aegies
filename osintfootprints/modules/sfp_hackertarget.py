...
# Purpose:      OsintFootprints plug-in to search HackerTarget.com for hosts sharing
...
class sfp_hackertarget(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'DOMAIN_NAME_PARENT':
            ...
            evt = OsintFootprintsEvent('RAW_DNS_RECORDS', "\n".join(records), self.__name__, event)
            ...
            for row in records:
                ...
                for host in set(hosts):
                    ...
                    evt = OsintFootprintsEvent(evt_type, host, self.__name__, event)
                    self.notifyListeners(evt)

                    if self.sf.isDomain(host, self.opts['_internettlds']):
                        if evt_type.startswith('AFFILIATE'):
                            evt = OsintFootprintsEvent('AFFILIATE_DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
                        else:
                            evt = OsintFootprintsEvent('DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
        ...
        for ip in qrylist:
            ...
            for h in hosts:
                ...
                if h not in myres and h != ip:
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        ...
                        # link to the co-host.
                        if eventName == "NETBLOCK_OWNER":
                            ipe = OsintFootprintsEvent("IP_ADDRESS", ip, self.__name__, event)
                            self.notifyListeners(ipe)
                            evt = OsintFootprintsEvent("CO_HOSTED_SITE", h.lower(), self.__name__, ipe)
                            self.notifyListeners(evt)
                        else:
                            evt = OsintFootprintsEvent("CO_HOSTED_SITE", h.lower(), self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if eventName == 'NETBLOCK_OWNER':
                pevent = OsintFootprintsEvent("IP_ADDRESS", ip, self.__name__, event)
            ...
            if self.opts.get('http_headers', True):
                ...
                if http_headers is not None:
                    e = OsintFootprintsEvent('WEBSERVER_HTTPHEADERS', json.dumps(http_headers), self.__name__, pevent)
                    e.actualSource = ip
