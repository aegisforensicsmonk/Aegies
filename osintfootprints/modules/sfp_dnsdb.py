...
# -------------------------------------------------------------------------------
# Name:        sfp_dnsdb
# Purpose:     OsintFootprints plug-in that resolves and gets history of domains and IPs
#
...
class sfp_dnsdb(OsintFootprintsPlugin):
    meta = {
        "name": "DNSDB",
    ...
    def handleEvent(self, event):
        ...
        if eventName == "DOMAIN_NAME":
            ...
            evt = OsintFootprintsEvent("RAW_RIR_DATA", str(rrsetRecords), self.__name__, event)
            ...
            for record in rrsetRecords:
                ...
                for data in record.get("rdata"):
                    ...
                    if record.get("rrtype") == "A":
                        ...
                        evt = OsintFootprintsEvent("IP_ADDRESS", data, self.__name__, event)
                    ...
                    if record.get("rrtype") == "AAAA":
                        ...
                        evt = OsintFootprintsEvent("IPV6_ADDRESS", data, self.__name__, event)
                    elif record.get("rrtype") == "MX":
                        data = re.sub(r'.*\s+(.*)', r'\1', data)
                        evt = OsintFootprintsEvent("PROVIDER_MAIL", data, self.__name__, event)
                    elif record.get("rrtype") == "NS":
                        evt = OsintFootprintsEvent("PROVIDER_DNS", data, self.__name__, event)
                    elif record.get("rrtype") == "TXT":
                        data = data.replace('"', '')
                        evt = OsintFootprintsEvent("DNS_TEXT", data, self.__name__, event)
            ...
            evt = OsintFootprintsEvent("RAW_RIR_DATA", str(rdataRecords), self.__name__, event)
            self.notifyListeners(evt)
            ...
            for record in rdataRecords:
                ...
                if self.opts["verify"] and not self.sf.resolveHost(data) and not self.sf.resolveHost6(data):
                    ...
                    evt = OsintFootprintsEvent("INTERNET_NAME_UNRESOLVED", data, self.__name__, event)
                else:
                    evt = OsintFootprintsEvent("INTERNET_NAME", data, self.__name__, event)
                self.notifyListeners(evt)
        ...
        for co in coHosts:
            ...
            if self.cohostcount < self.opts["maxcohost"]:
                evt = OsintFootprintsEvent("CO_HOSTED_SITE", co, self.__name__, event)
                self.notifyListeners(evt)
                self.cohostcount += 1
