...
class sfp_focsec(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        if is_bot:
            e = OsintFootprintsEvent("MALICIOUS_IPADDR", f"Focsec [{eventData}]", self.__name__, event)
            self.notifyListeners(e)

        is_tor = data.get('is_tor')
        if is_tor:
            e = OsintFootprintsEvent("TOR_EXIT_NODE", eventData, self.__name__, event)
            self.notifyListeners(e)

        is_vpn = data.get('is_vpn')
        if is_vpn:
            e = OsintFootprintsEvent("VPN_HOST", eventData, self.__name__, event)
            self.notifyListeners(e)

        is_proxy = data.get('is_proxy')
        if is_proxy:
            e = OsintFootprintsEvent("PROXY_HOST", eventData, self.__name__, event)
            self.notifyListeners(e)
        ...
        if location:
            e = OsintFootprintsEvent("GEOINFO", location, self.__name__, event)
            self.notifyListeners(e)
