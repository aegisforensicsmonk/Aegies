...
class sfp_fsecure_riddler(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        for addr in set(addrs):
            if self.sf.validIP(addr):
                evt = OsintFootprintsEvent('IP_ADDRESS', addr, self.__name__, event)
                self.notifyListeners(evt)

        for coord in set(coords):
            evt = OsintFootprintsEvent('PHYSICAL_COORDINATES', coord, self.__name__, event)
            self.notifyListeners(evt)
