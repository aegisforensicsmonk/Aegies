...
class sfp_clearbit(OsintFootprintsPlugin):
                ...
                if site:
                    if 'phoneNumbers' in site:
                        for p in site['phoneNumbers']:
                            evt = OsintFootprintsEvent("PHONE_NUMBER", p, self.__name__, event)
                            self.notifyListeners(evt)
                    ...
                    if 'emailAddresses' in company['site']:
                        for e in site['emailAddresses']:
                            if e.split("@")[0] in self.opts['_genericusers'].split(","):
                                ...
                                evttype = "EMAILADDR"
                            evt = OsintFootprintsEvent(evttype, e, self.__name__, event)
                            self.notifyListeners(evt)
