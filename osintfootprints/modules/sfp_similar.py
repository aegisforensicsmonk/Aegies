...
# -------------------------------------------------------------------------------
# Name:         sfp_similar
# Purpose:      OsintFootprints plug-in for identifying domains that look similar
...
class sfp_similar(OsintFootprintsPlugin):
            ...
            try:
                for domain in [f"{d}{tld}", f"www.{d}{tld}"]:
                    if self.sf.resolveHost(domain) or self.sf.resolveHost6(domain):
                        ...
                        evt = OsintFootprintsEvent("SIMILARDOMAIN", f"{d}{tld}", self.__name__, event)
