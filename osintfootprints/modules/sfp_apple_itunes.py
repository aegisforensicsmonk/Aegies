...
class sfp_apple_itunes(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        for host in set(hosts):
            ...
            if self.getTarget().matches(host, includeChildren=True, includeParents=True):
                ...
                evt = OsintFootprintsEvent('AFFILIATE_INTERNET_NAME', host, self.__name__, event)
                self.notifyListeners(evt)
            found = True

        if found:
            evt = OsintFootprintsEvent('RAW_RIR_DATA', json.dumps(data), self.__name__, event)
            self.notifyListeners(evt)
