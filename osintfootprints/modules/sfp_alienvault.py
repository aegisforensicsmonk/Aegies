...
class sfp_alienvault(OsintFootprintsPlugin):
        ...
        if eventName == 'INTERNET_NAME':
            ...
            for url in set(urls):
                ...
                if url not in self.results:
                    ...
                    evt = OsintFootprintsEvent('LINKED_URL_INTERNAL', url, self.__name__, event)
                ...
                if passive_dns:
                    ...
                    for rec in passive_dns:
                        ...
                        if self.getTarget().matches(host, includeParents=True):
                            ...
                            evt = OsintFootprintsEvent(evtType, host, self.__name__, event)
                            self.notifyListeners(evt)
                            continue
        ...
        for addr in qrylist:
            ...
            if rec.get("reputation", None):
                ...
                if eventName == 'NETBLOCKV6_OWNER':
                    ...
                    pevent = OsintFootprintsEvent("AFFILIATE_IPV6_ADDRESS", addr, self.__name__, event)
                    self.notifyListeners(pevent)
                else:
                    pevent = event

                e = OsintFootprintsEvent(evtType, descr, self.__name__, pevent)
                self.notifyListeners(e)
