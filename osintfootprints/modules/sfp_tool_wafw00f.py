...
# Purpose:     OsintFootprints plug-in for using the WAFW00F tool.
...
class sfp_tool_wafw00f(OsintFootprintsPlugin):
    meta = {
        'name': "Tool - WAFW00F",
    ...
    def handleEvent(self, event):
        ...
        if not OsintFootprintsHelpers.sanitiseInput(url):
        ...
        evt = OsintFootprintsEvent('RAW_RIR_DATA', json.dumps(result_json), self.__name__, event)
        ...
        for waf in result_json:
            ...
            if software:
                evt = OsintFootprintsEvent('WEBSERVER_TECHNOLOGY', software, self.__name__, event)
                self.notifyListeners(evt)
