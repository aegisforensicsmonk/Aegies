...
# Purpose:     OsintFootprints plug-in to search NeutrinoAPI for IP address info,
...
class sfp_neutrinoapi(OsintFootprintsPlugin):
        ...
        if eventName == 'PHONE_NUMBER':
            ...
            if data is None:
                ...
                if data.get('location') is not None and data.get('country') is not None:
                    ...
                    evt = OsintFootprintsEvent("RAW_RIR_DATA", str(data), self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            if data is None:
                ...
                if data.get('is-listed'):
                    evt = OsintFootprintsEvent("MALICIOUS_IPADDR", f"NeutrinoAPI - IP Blocklist [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = OsintFootprintsEvent("BLACKLISTED_IPADDR", f"NeutrinoAPI - IP Blocklist [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = OsintFootprintsEvent("RAW_RIR_DATA", str(data), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if data.get('is-listed'):
                    evt = OsintFootprintsEvent("MALICIOUS_IPADDR", f"NeutrinoAPI - Host Reputation [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = OsintFootprintsEvent("BLACKLISTED_IPADDR", f"NeutrinoAPI - Host Reputation [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = OsintFootprintsEvent("RAW_RIR_DATA", str(data), self.__name__, event)
                    self.notifyListeners(evt)
