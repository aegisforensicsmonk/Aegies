...
class sfp_robtex(OsintFootprintsPlugin):
        ...
        for ip in qrylist:
            ...
            for r in data.get('pas'):
                ...
                if eventName == "NETBLOCK_OWNER":
                    ...
                    evt = OsintFootprintsEvent("CO_HOSTED_SITE", host, self.__name__, ipe)
                    self.notifyListeners(evt)
                elif eventName == "NETBLOCKV6_OWNER":
                    ipe = OsintFootprintsEvent("IPV6_ADDRESS", ip, self.__name__, event)
                    self.notifyListeners(ipe)
                    evt = OsintFootprintsEvent("CO_HOSTED_SITE", host, self.__name__, ipe)
                    self.notifyListeners(evt)
                else:
                    evt = OsintFootprintsEvent("CO_HOSTED_SITE", host, self.__name__, event)
                    self.notifyListeners(evt)
