...
# -------------------------------------------------------------------------------
# Name:        sfp_opnenic
# Purpose:     OsintFootprints plug-in for resolving host names on the OpenNIC
...
class sfp_opennic(OsintFootprintsPlugin):
        ...
        for addr in set(addrs):
            if self.sf.validIP(addr):
                if affiliate and not self.getTarget().matches(addr, includeParents=True):
                    evt = OsintFootprintsEvent("AFFILIATE_IPADDR", addr, self.__name__, event)
                else:
                ...
                if affiliate and not self.getTarget().matches(addr, includeParents=True):
                    evt = OsintFootprintsEvent("AFFILIATE_IPV6_ADDRESS", addr, self.__name__, event)
                else:
                    evt = OsintFootprintsEvent("IPV6_ADDRESS", addr, self.__name__, event)
