...
class sfp_dehashed(OsintFootprintsPlugin):
        ...
        while currentPage <= maxPages:
            ...
            for row in data.get('entries'):
                ...
                if eventName == "EMAILADDR":
                    if email == eventData:
                        evt = OsintFootprintsEvent('EMAILADDR_COMPROMISED', f"{email} [{leakSource}]", self.__name__, event)
                        self.notifyListeners(evt)

                        if password:
                            evt = OsintFootprintsEvent('PASSWORD_COMPROMISED', f"{email}:{password} [{leakSource}]", self.__name__, event)
                            self.notifyListeners(evt)

                        if passwordHash:
                            evt = OsintFootprintsEvent('HASH_COMPROMISED', f"{email}:{passwordHash} [{leakSource}]", self.__name__, event)
                            self.notifyListeners(evt)

                        evt = OsintFootprintsEvent('RAW_RIR_DATA', str(row), self.__name__, event)
                        self.notifyListeners(evt)

                if eventName == "DOMAIN_NAME":
                    pevent = OsintFootprintsEvent("EMAILADDR", email, self.__name__, event)
                    if email not in emailResults:
                        self.notifyListeners(pevent)
                        emailResults.add(email)

                    evt = OsintFootprintsEvent('EMAILADDR_COMPROMISED', f"{email} [{leakSource}]", self.__name__, pevent)
                    self.notifyListeners(evt)

                    if password:
                        evt = OsintFootprintsEvent('PASSWORD_COMPROMISED', f"{email}:{password} [{leakSource}]", self.__name__, pevent)
                        self.notifyListeners(evt)

                    if passwordHash:
                        evt = OsintFootprintsEvent('HASH_COMPROMISED', f"{email}:{passwordHash} [{leakSource}]", self.__name__, pevent)
                        self.notifyListeners(evt)

                    evt = OsintFootprintsEvent('RAW_RIR_DATA', str(row), self.__name__, pevent)
                    self.notifyListeners(evt)
