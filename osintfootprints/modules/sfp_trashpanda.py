...
# -------------------------------------------------------------------------------
# Name:         sfp_trashpanda
# Purpose:      Osintfootprints plugin to query Trashpanda - got-hacked.wtf API to gather intelligence about
...
class sfp_trashpanda(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        leaksiteUrls = set()
        for row in data:
            evt = OsintFootprintsEvent("PASSWORD_COMPROMISED", f"{row.get('email')}:{row.get('password')} [{row.get('paste')}]", self.__name__, event)
        ...
        for leaksiteUrl in leaksiteUrls:
            try:
                ...
                evt = OsintFootprintsEvent("LEAKSITE_URL", leaksiteUrl, self.__name__, event)
                self.notifyListeners(evt)

                evt = OsintFootprintsEvent("LEAKSITE_CONTENT", res['content'], self.__name__, evt)
                self.notifyListeners(evt)
