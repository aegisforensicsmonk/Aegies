...
# -------------------------------------------------------------------------------
# Name:         sfp_spider
# Purpose:      OsintFootprints plug-in for spidering sites and returning meta data
...
class sfp_spider(OsintFootprintsPlugin):
    ...
    def processUrl(self, url: str) -> dict:
        ...
        links = OsintFootprintsHelpers.extractLinksFromHtml(
            url,
            data,
        ...
        for link in links:
            if not self.opts['reportduplicates']:
                if link in self.urlEvents:
                    continue
            # Supply the OsintFootprintsEvent of the parent URL as the parent
    ...
    def cleanLinks(self, links: list) -> list:
        ...
        for link in links:
            linkBase = OsintFootprintsHelpers.urlBaseUrl(link)
    ...
    def linkNotify(self, url: str, parentEvent=None):
        ...
        event = OsintFootprintsEvent(utype, url, self.__name__, parentEvent)
    ...
    def spiderFrom(self, startingPoint: str) -> None:
        ...
        if self.opts['robotsonly']:
            targetBase = OsintFootprintsHelpers.urlBaseUrl(startingPoint)
            if targetBase not in self.robotsRules:
                ...
                if res:
                    ...
                    if robots_txt:
                        ...
                        self.robotsRules[targetBase] = OsintFootprintsHelpers.extractUrlsFromRobotsTxt(robots_txt)
