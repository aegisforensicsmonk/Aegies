...
from osintfootprints import OsintFootprintsEvent, OsintFootprintsPlugin, OsintFootprintsHelpers


class sfp_sslcert(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        try:
            ...
            pemcert = OsintFootprintsHelpers.sslDerToPem(dercert)
        ...
        if eventName in ['INTERNET_NAME', 'IP_ADDRESS']:
            evt = OsintFootprintsEvent('TCP_PORT_OPEN', fqdn + ':' + str(port), self.__name__, event)
        ...
        rawevt = OsintFootprintsEvent("SSL_CERTIFICATE_RAW", cert['text'], self.__name__, event)
        self.notifyListeners(rawevt)

        if cert.get('issued'):
            evt = OsintFootprintsEvent('SSL_CERTIFICATE_ISSUED', cert['issued'], self.__name__, event)
            self.notifyListeners(evt)

        if cert.get('issuer'):
            evt = OsintFootprintsEvent('SSL_CERTIFICATE_ISSUER', cert['issuer'], self.__name__, event)
            self.notifyListeners(evt)

        if eventName != "IP_ADDRESS" and cert.get('mismatch'):
            evt = OsintFootprintsEvent('SSL_CERTIFICATE_MISMATCH', ', '.join(cert.get('hosts')), self.__name__, event)
            self.notifyListeners(evt)
        ...
        for san in set(cert.get('altnames', list())):
            ...
            evt = OsintFootprintsEvent(evt_type, domain, self.__name__, event)
            self.notifyListeners(evt)

            if self.sf.isDomain(domain, self.opts['_internettlds']):
                if evt_type == 'CO_HOSTED_SITE':
                    evt = OsintFootprintsEvent('CO_HOSTED_SITE_DOMAIN', domain, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = OsintFootprintsEvent('DOMAIN_NAME', domain, self.__name__, event)
                    self.notifyListeners(evt)

        if cert.get('expired'):
            evt = OsintFootprintsEvent("SSL_CERTIFICATE_EXPIRED", cert.get('expirystr', 'Unknown'), self.__name__, event)
            self.notifyListeners(evt)
            return

        if cert.get('expiring'):
            evt = OsintFootprintsEvent("SSL_CERTIFICATE_EXPIRING", cert.get('expirystr', 'Unknown'), self.__name__, event)
            self.notifyListeners(evt)
