...
class sfp_accounts(OsintFootprintsPlugin):
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        self.commonNames = OsintFootprintsHelpers.humanNamesFromWordlists()
        self.words = OsintFootprintsHelpers.dictionaryWordsFromWordlists()

        content = self.sf.cacheGet("sfaccountsv2", 48)
        if content is None:
            url = "https://raw.githubusercontent.com/WebBreacher/WhatsMyName/main/wmn-data.json"
            data = self.sf.fetchUrl(url, useragent="OsintFootprints")
    ...
    def handleEvent(self, event):
        ...
        for user in set(users):
            ...
            if user not in self.reportedUsers and eventData != user:
                ...
                evt = OsintFootprintsEvent("USERNAME", user, self.__name__, event)
