...
from osintfootprints import OsintFootprintsEvent, OsintFootprintsHelpers, OsintFootprintsPlugin


class sfp_jsonwhoiscom(OsintFootprintsPlugin):
    ...
    def handleEvent(self, event):
        ...
        evt = OsintFootprintsEvent('RAW_RIR_DATA', str(res), self.__name__, event)
        ...
        for contact in contacts:
            ...
            if email:
                if OsintFootprintsHelpers.validEmail(email):
                    emails.append(email)
            ...
            country = OsintFootprintsHelpers.countryNameFromCountryCode(contact.get('country_code'))
        ...
        for email in set(emails):
            ...
            if self.getTarget().matches(mail_domain, includeChildren=True):
                if email.split("@")[0] in self.opts['_genericusers'].split(","):
                    ...
                    evttype = "EMAILADDR"
                evt = OsintFootprintsEvent(evttype, email, self.__name__, event)
                self.notifyListeners(evt)
            else:
                evt = OsintFootprintsEvent("AFFILIATE_EMAILADDR", email, self.__name__, event)
                self.notifyListeners(evt)

        if eventName in ["DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = OsintFootprintsEvent("DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)
            ...
            if registrar:
                ...
                if registrar_name:
                    evt = OsintFootprintsEvent("DOMAIN_REGISTRAR", registrar_name, self.__name__, event)
                    self.notifyListeners(evt)

            for dns_provider in set(dns_providers):
                evt = OsintFootprintsEvent("PROVIDER_DNS", dns_provider, self.__name__, event)
                self.notifyListeners(evt)

            for name in set(names):
                evt = OsintFootprintsEvent("RAW_RIR_DATA", f"Possible full name {name}", self.__name__, event)
                self.notifyListeners(evt)

            for phone in set(phones):
                evt = OsintFootprintsEvent("PHONE_NUMBER", phone, self.__name__, event)
                self.notifyListeners(evt)

            for location in set(locations):
                evt = OsintFootprintsEvent("PHYSICAL_ADDRESS", location, self.__name__, event)
                self.notifyListeners(evt)

        if eventName in ["AFFILIATE_DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = OsintFootprintsEvent("AFFILIATE_DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)

            available = res.get('available?')
            if available:
                evt = OsintFootprintsEvent("AFFILIATE_DOMAIN_UNREGISTERED", eventData, self.__name__, event)
                self.notifyListeners(evt)
