...
# -------------------------------------------------------------------------------
# Name:        sfp_networksdb
# Purpose:     OsintFootprints plug-in to search NetworksDB.io API for IP address and
#              domain information.
#
...
class sfp_networksdb(OsintFootprintsPlugin):
            ...
            if data is None:
                ...
                if network:
                    ...
                    if cidr and cidr != 'N/A' and self.sf.validIpNetwork(cidr):
                        if ":" in cidr:
                            evt = OsintFootprintsEvent('NETBLOCKV6_MEMBER', cidr, self.__name__, event)
                        else:
                            evt = OsintFootprintsEvent('NETBLOCK_MEMBER', cidr, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            for co in set(cohosts):
                ...
                if not self.opts['cohostsamedomain']:
                    if self.getTarget().matches(co, includeParents=True):
                        evt = OsintFootprintsEvent('INTERNET_NAME', co, self.__name__, event)
                        self.notifyListeners(evt)
                        if self.sf.isDomain(co, self.opts['_internettlds']):
                            evt = OsintFootprintsEvent('DOMAIN_NAME', co, self.__name__, event)
                            self.notifyListeners(evt)
                        continue
        ...
        if eventName in ["INTERNET_NAME", "DOMAIN_NAME"]:
            ...
            for ip in res:
                if self.sf.validIP(ip):
                    evt = OsintFootprintsEvent('IP_ADDRESS', ip, self.__name__, event)
                    self.notifyListeners(evt)
                elif self.sf.validIP6(ip):
                    evt = OsintFootprintsEvent('IPV6_ADDRESS', ip, self.__name__, event)
                    self.notifyListeners(evt)
