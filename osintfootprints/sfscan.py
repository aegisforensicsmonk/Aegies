...
from osintfootprints import OsintFootprintsDb, OsintFootprintsEvent, OsintFootprintsPlugin, OsintFootprintsTarget, OsintFootprintsHelpers, OsintFootprintsThreadPool, OsintFootprintsCorrelator, logger


def startOsintFootprintsScanner(loggingQueue, *args, **kwargs):
    logger.logWorkerSetup(loggingQueue)
    return OsintFootprintsScanner(*args, **kwargs)


class OsintFootprintsScanner():
    """OsintFootprintsScanner object.
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True) -> None:
        """Initialize OsintFootprintsScanner object.
        ...
        self.__dbh = OsintFootprintsDb(self.__config)
        ...
        self.__sf = OsintFootprints(self.__config)
        ...
        if scanId:
            ...
            self.__scanId = OsintFootprintsHelpers.genScanInstanceId()
        ...
        # Create our target
        try:
            self.__target = OsintFootprintsTarget(self.__targetValue, self.__targetType)
        ...
        self.__sharedThreadPool = OsintFootprintsThreadPool(threads=self.__config.get("_maxthreads", 3), name='sharedThreadPool')
    ...
    def __startScan(self) -> None:
        ...
        try:
            ...
            psMod = OsintFootprintsPlugin()
            psMod.__name__ = "OsintFootprints UI"
            ...
            rootEvent = OsintFootprintsEvent("ROOT", self.__targetValue, "", None)
            psMod.notifyListeners(rootEvent)
            firstEvent = OsintFootprintsEvent(self.__targetType, self.__targetValue,
                                         "OsintFootprints UI", rootEvent)
            ...
            if self.__targetType == 'INTERNET_NAME' and self.__sf.isDomain(self.__targetValue, self.__config['_internettlds']):
                firstEvent = OsintFootprintsEvent('DOMAIN_NAME', self.__targetValue, "OsintFootprints UI", rootEvent)
                psMod.notifyListeners(firstEvent)
    ...
    def runCorrelations(self) -> None:
        ...
        corr = OsintFootprintsCorrelator(self.__dbh, ruleset, self.__scanId)
